...
(define (c10h number base list)
    (if (= number 0)
        list
        (c10h (quotient number base) base (cons (modulo number base) list))))
...
(define (b10h oldbase numlist int)
  (if (null? numlist)
      (quotient int oldbase)
      (b10h oldbase (cdr numlist) (* (+ int (car numlist)) oldbase))))

(define (base10 oldbase numlist)
  (b10h oldbase numlist 0))
...
(define (gh cur_num ratio n sequence)
  (if (= n -1)
      sequence
      (gh cur_num ratio (- n 1) (cons (* cur_num (expt ratio n)) sequence))))

(define (geo-prog cur_num ratio n)
  (gh cur_num ratio (- n 1) '()))